...
def process(inputDir,basePath):
    ...
    assert(analyzeStartFrame is not None and analyzeEndFrame is not None)

    # Read pose of marker board
    rVec,tVec = utils.getMarkerBoardPose(inputDir / 'boardPose.tsv',analyzeStartFrame,analyzeEndFrame,True)

    # Read gaze on board data
    gazeWorld = utils.getGazeWorldData(inputDir / 'gazeWorldPos.tsv',analyzeStartFrame,analyzeEndFrame,True)
    ...
    # 2. collect data
    data = {}
    data['time'] = np.array([s.ts for v in gazeWorld.values() for s in v])-gazeWorld[analyzeStartFrame][0].ts
    data['L_X']  = np.array([s.lGaze2D[0] for v in gazeWorld.values() for s in v])
    data['L_Y']  = np.array([s.lGaze2D[1] for v in gazeWorld.values() for s in v])
    data['R_X']  = np.array([s.rGaze2D[0] for v in gazeWorld.values() for s in v])
    data['R_Y']  = np.array([s.rGaze2D[1] for v in gazeWorld.values() for s in v])
    ...
    offset[:] = np.nan
    frameIdxs = [k for k,v in gazeWorld.items() for s in v]
    oriL  = np.vstack([s.lGazeOrigin for v in gazeWorld.values() for s in v])
    oriR  = np.vstack([s.rGazeOrigin for v in gazeWorld.values() for s in v])
    gaze3L= np.vstack([s.lGaze3D     for v in gazeWorld.values() for s in v])
    gaze3R= np.vstack([s.rGaze3D     for v in gazeWorld.values() for s in v])
    gaze2L= np.vstack([s.lGaze2D     for v in gazeWorld.values() for s in v])
    gaze2R= np.vstack([s.rGaze2D     for v in gazeWorld.values() for s in v])
    ...
    df.to_csv(str(inputDir / 'dataQuality.tsv'), sep='\t', na_rep='nan', float_format="%.3f")
